...
function checkIsNew(mac) {
    ...
    let known = JSON.parse(localStorage.getItem('known_macs') || '[]');
    return !known.includes(cleanMac);
}
...
function startScan() {
    ...
    const known = JSON.parse(localStorage.getItem('known_macs') || '[]');
    isFirstRun = (known.length === 0);
    
    log("INITIALIZING_DEEP_SCAN...");
    
    const backendUrl = window.location.port === "5005" ? "" : "http://127.0.0.1:5005";
    const source = new EventSource(backendUrl + "/scan");
    let devices = 0;
    ...
    source.onmessage = (event) => {
        ...
        const isNew = isFirstRun ? false : checkIsNew(data.mac);
        
        if (isFirstRun) {
            let currentKnown = JSON.parse(localStorage.getItem('known_macs') || '[]');
            const cleanMac = data.mac.replace(/[^\w\:]/g, '');
            if (cleanMac !== "00:00:00:00:00:00" && !currentKnown.includes(cleanMac)) {
                currentKnown.push(cleanMac);
                localStorage.setItem('known_macs', JSON.stringify(currentKnown));
            }
        }
