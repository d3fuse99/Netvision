...
function trustAllCurrent() {
    ...
    currentSessionData.forEach(device => {
        const cleanMac = device.mac.replace(/[^\w\:]/g, '');
        if (cleanMac !== "00:00:00:00:00:00" && !known.includes(cleanMac)) {
            known.push(cleanMac);
        }
        const cleanMacId = cleanMac.replace(/:/g, '');
        const card = document.getElementById(`dev-${cleanMacId}`);
        if (card) {
...
function checkIsNew(mac) {
    ...
    let known = [];
    try {
        const stored = localStorage.getItem('known_macs');
        if (stored === null) {
            return false;
        }
        known = JSON.parse(stored);
    } catch (e) {
        known = [];
    }
...
function startScan() {
    ...
    const source = new EventSource(`/scan?threads=${threads}`);
    let devices = 0;
    ...
    source.onmessage = (event) => {
        ...
        if (data.done) {
            ...
            if (localStorage.getItem('known_macs') === null) {
                trustAllCurrent();
            }
            source.close();
            return;
